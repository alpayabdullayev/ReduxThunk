...
const CategoriesSlice = createSlice({
  ...
  reducers: {
    addBasket: (state, action) => {
      const elementIndex = state.basket.findIndex(
    ...
    addWishlist: (state, action) => {
        const isItemInWishlist = state.wishlist.findIndex((x) => x.id === action.payload.id);
      
        if (isItemInWishlist !== -1) {
          const newWishlist = state.wishlist.filter((x) => x.id !== action.payload.id);
...
export const { addBasket,addWishlist } = CategoriesSlice.actions;
